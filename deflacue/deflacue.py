...
class Deflacue:
    ...
    def process_cue(self, *, cue_file: Path, target_path: Path,delete=False,music,img,needImage=False):
        ...
        for track in tracks:
            track_file = str(target_path)+"/"+str(track.file.path)
            if not os.path.exists(track_file):
                if not os.path.exists(music[0]):
                    track_file = str(target_path) + "/" + str(track.file.path).split(".")[0] + ".wav"
                else:
                    if len(music) == 1:
                        track_file = music[0]
                if not os.path.exists(track_file):
            ...
            filename = f"{track_num} - {sanitize(fix_name(track.title))}.flac"
            ...
            audio = FLAC(target_path +"/"+ filename)

            audio['TITLE'] = str(track.data.get("TITLE",""))
            audio['PERFORMER'] = str(track.data.get("PERFORMER",""))
            audio['DATE'] = str(track.data.get("DATE", ""))
            audio['ALBUM'] = str(track.data.get("ALBUM", ""))
            if needImage and len(img)>0:
        ...
        if delete:
            try:
                if not os.path.exists(target_path +"/"+ filename):
                    LOGGER.info(f'\n处理错误，已取消删除作业 `{cue_file}`\n')
                    return
                os.remove(cue_file)
                os.remove(str(track_file))
            except:
                a = 1
